...
class Model(object):
    ...
    def pre_compute(self,compute_py = True):
        ...
        self.N=len(self.eta)
        if not hasattr(self,"actions_index"):
            self.actions_index=range(len(self.eta))
    def get_costs(self):
    ...
    def get_parent_assignments(self):
        if not hasattr(self,"parent_assignments") or self.parent_assignments is None:
            self.parent_assignments = self.generate_binary_assignments(self.N)
    ...
    def _minimize_addConstraints(self,tol,options):
        # when i not in set actions, etai=0
        # use _minimize_addConstrains to replace _minimize by using index
        actions_index=self.actions_index
        total=range(self.K)
        diff_actions_index=list(set(total).difference(set(actions_index)))
        eta = np.random.random(self.K)
        eta[diff_actions_index]=0
        eta0= eta/eta.sum()
        constr_A=np.eye(self.K)
        constr_A[actions_index,actions_index]=0
        self.constr_A=constr_A
        
        constraints=({'type':'eq','fun':lambda eta: eta.sum()-1.0},
                      {'type':'eq','fun': lambda eta: np.dot(self.constr_A,eta)})
        res = minimize(self.m_eta, eta0,bounds = [(0.0,1.0)]*self.K, constraints = constraints, method='SLSQP',options = options)
        return res    
    ...
    def find_eta(self,tol = 1e-10,min_starts = 1, max_starts = 10,  options={'disp': True, 'maxiter':200}):
        ...
        while success < min_starts and starts < max_starts:
            res = self._minimize_addConstraints(tol,options)            
